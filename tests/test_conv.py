...
class TestUtils:
    def test_conv_factor(self):
        ...
        assert res * 1e6 == pytest.approx(
            1.184455, abs=1e-3
        )  # Convert to m³ for comparison
    ...
    def test_convert_series(self):
        ...
        assert res["2020-01"].iloc[0] == pytest.approx(
            12016.129, abs=1
        )  # Round to units
    ...
    def test_aliases(self):
        ...
        # 'propane' is now a first-class commodity (was previously an alias for
        # 'lpg'); the two have different densities so factors differ. Verify each
        # resolves to its own commodity entry rather than the same one.
        ...
        # Lighter NGL species => more barrels per tonne than the generic LPG blend
        assert propane_factor > lpg_factor

        # NGL-species aliases: n_butane / n-butane / normal_butane -> butane
        for alias in ("n_butane", "n-butane", "normal_butane"):
            assert convfactors.convfactor("kt", "bbl", alias) == convfactors.convfactor(
                "kt", "bbl", "butane"
            )
        # iso_butane / i-butane -> isobutane
        for alias in ("iso_butane", "iso-butane", "i_butane", "i-butane"):
            assert convfactors.convfactor("kt", "bbl", alias) == convfactors.convfactor(
                "kt", "bbl", "isobutane"
            )
    ...
    def test_natural_gas_special_cases(self):
        ...
        assert res == pytest.approx(
            36000000, abs=1000
        )  # 1 BCM * 0.036 GJ/m³ = 36 million GJ = 0.036 PJ
