...
class TestTransforms(unittest.TestCase):
    def test_seasonalise2(self):
        ...
        self.assertEqual(
            seas[2020].loc[pd.to_datetime("%s-03-13" % dates.curyear)], 31.73
        )
        self.assertEqual(
            seas[2020].loc[pd.to_datetime("%s-03-16" % dates.curyear)], 28.7
        )
        self.assertEqual(
            seas[2020].loc[pd.to_datetime("%s-03-17" % dates.curyear)], 26.95
        )
        self.assertEqual(
            seas[2020].loc[pd.to_datetime("%s-03-18" % dates.curyear)], 20.37
        )
        self.assertEqual(
            seas[2020].loc[pd.to_datetime("%s-03-19" % dates.curyear)], 25.22
        )
