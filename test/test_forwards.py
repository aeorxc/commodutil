...
def test_timespreads(contracts):
    ...
    assert res[2019].loc[pd.to_datetime("2019-01-02")] == pytest.approx(-1.51, 2)
    assert res[2019].loc[pd.to_datetime("2019-05-21")] == pytest.approx(0.37, 2)

    res = forwards.time_spreads(contracts, m1=12, m2=12)
    assert res[2019].loc[pd.to_datetime("2019-11-20")] == pytest.approx(3.56, 2)
    assert res[2020].loc[pd.to_datetime("2019-03-20")] == pytest.approx(2.11, 2)


def test_timespreads_quaterly(contracts):
    res = forwards.time_spreads(contracts, m1="Q1", m2="Q2")
    assert res[2020].loc[pd.to_datetime("2019-01-02")] == pytest.approx(-1.51, 2)
    assert res[2020].loc[pd.to_datetime("2019-05-21")] == pytest.approx(0.37, 2)

    res = forwards.time_spreads(contracts, m1="Q4", m2="Q1")
    assert res[2020].loc[pd.to_datetime("2019-01-02")] == pytest.approx(-0.25, 2)
    assert res[2020].loc[pd.to_datetime("2019-05-21")] == pytest.approx(0.91, 2)


def test_spread_combinations(contracts):
...
def test_continuous_futures(contracts):
    ...
    assert res["M1"].loc[pd.to_datetime("2020-11-20")] == pytest.approx(42.15, 3)
    assert res["M1"].loc[pd.to_datetime("2020-11-23")] == pytest.approx(43.06, 3)
    assert res["M2"].loc[pd.to_datetime("2020-11-19")] == pytest.approx(41.90, 3)
    assert res["M2"].loc[pd.to_datetime("2020-11-20")] == pytest.approx(42.42, 3)
    assert res["M2"].loc[pd.to_datetime("2020-11-23")] == pytest.approx(43.28, 3)

    res = forwards.continuous_futures(cl, expiry_dates=expiry_dates, roll_days=1)
    assert res["M1"].loc[pd.to_datetime("2020-11-19")] == pytest.approx(41.74, 3)
    assert res["M1"].loc[pd.to_datetime("2020-11-20")] == pytest.approx(42.42, 3)
    assert res["M1"].loc[pd.to_datetime("2020-11-23")] == pytest.approx(43.06, 3)
    ...
    assert res["M2"].loc[pd.to_datetime("2020-11-19")] == pytest.approx(41.90, 3)
    assert res["M2"].loc[pd.to_datetime("2020-11-20")] == pytest.approx(42.64, 3)
    assert res["M2"].loc[pd.to_datetime("2020-11-23")] == pytest.approx(43.28, 3)

    res = forwards.continuous_futures(
        cl, expiry_dates=expiry_dates, front_month=1, back_adjust=True
    )
    assert res["M1"].loc[pd.to_datetime("2020-11-19")] == pytest.approx(42.01, 3)
    assert res["M1"].loc[pd.to_datetime("2020-11-20")] == pytest.approx(42.42, 3)
    assert res["M1"].loc[pd.to_datetime("2020-11-23")] == pytest.approx(43.06, 3)