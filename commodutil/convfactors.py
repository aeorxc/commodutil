...
def normalize_price_unit(unit: object) -> Optional[str]:
    """Return a price unit string suitable for ``convert_price``.

    This keeps vendor/product-specific denominator aliases out of callers that
    only need to convert a price observation into a target price unit.
    """
    if unit is None:
        return None
    text = str(unit).strip()
    ...
    currency, bare_unit = _currency.split_currency_unit(text)
    unit_token = _to_pint_token(bare_unit)
    if unit_token and unit_token.upper() == "LB_SOYBEANOIL":
        unit_token = "lb"
    if not currency:
        return unit_token
    return f"{currency}/{unit_token}"


def price_unit_from_quote(currency: object, quote_unit: object) -> Optional[str]:
    """Build a ``convert_price`` price-unit token from metadata fields."""
    unit = normalize_price_unit(quote_unit)
    if not unit:
        return None
...
def convert_price_from_quote(
    ...
    **kwargs,
    ...
    """Convert a price from metadata currency/quote-unit fields to a target unit.

    Returns ``(value, source_price_unit, normalized_target_price_unit, converted)``.
    If either side is missing, or the units already match, the original value is
